...
5. **Регистр `ПЛ_СоответствиеСФронтСистемами` (модуль «Общепит», референс) имеет собственный менеджер-модуль с готовыми экспортными точками**, не упомянутыми в `proposal.md`, но релевантными для проектирования формы сопоставления:
   ...
   - `ЗаполнитьСсылкиПоКодамПослеЗагрузкиВФоне(ФронтСистема, ИдентификаторФормы)` / `ЗаполнитьСсылкиПоКодамПослеЗагрузки(Параметры, АдресРезультата)` — постобработка: пробегает `питДанныеПродажФронта` с незаполненными ссылками, подставляет `Объект` из сопоставлений по кодам, ставит `ПЛ_СопоставлениеЗавершено`. **Ни один из прочитанных модулей нашего расширения (`MRS_МенеджерОбменаTNG`/`Micros`, `MRS_АРМ_Закрытия_смены`, `MRS_ЗагрузкаДанныхИзФронтСистем`) не вызывает эту функцию** — то есть либо её вызывает что-то в основной конфигурации/БСП вне видимости этого репозитория, либо она не вызывается вовсе автоматически. Это новый факт, не зафиксированный в `proposal.md`; см. `## Open Questions`.
...
Используется для трёх интерактивных операций мастера:
...
```
```text
Diagram: Checks (background, unchanged) -> interactive generation from wizard (sequence)
...
2. **Пропагация результата сопоставления объектов фронта на уже загруженные чеки — статус неопределён, но НЕ через `ЗаполнитьСсылкиПоКодамПослеЗагрузки`.** Экспортная точка менеджера регистра `ПЛ_СоответствиеСФронтСистемами.ЗаполнитьСсылкиПоКодамПослеЗагрузки[ВФоне]` (см. `Discovered Patterns`, была п.5) в этой редакции НЕ вызывается решением 4 (прежний хук, куда она была добавлена как защитная мера, отменён целиком). Вопрос остаётся открытым в ДРУГОМ контексте — шаг «Сопоставление объектов фронта»: `proposal.md → Flows`, п.4 предполагает, что после того как пользователь донастроил сопоставление, именно повторное действие «(Пере)загрузить чеки за день» (не отдельный push резолвинга) подтягивает новые объекты в уже загруженные чеки. Не подтверждено чтением кода в этой сессии: действительно ли повторный вызов `MRS_МенеджерОбменаTNG`/`MRS_МенеджерОбменаMicros` перезаписывает/дозаполняет ссылки на УЖЕ загруженные документы `питДанныеПродажФронта` (используя обновлённое `ПЛ_СоответствиеСФронтСистемами`), или только загружает НОВЫЕ, не встречавшиеся чеки, оставляя старые с текстовыми кодами нерезолвленными. Если верно последнее — потребуется явный вызов `ЗаполнитьСсылкиПоКодамПослеЗагрузки` (или аналога) именно на шаге «Сопоставление» после точечной правки соответствия, а не в решении 4. Не может быть закрыт сейчас (модуль «Общепит»/основная конфигурация не индексированы MCP-инструментами этой сессии) — уточняется на тестовой ИБ при реализации шага «Сопоставление» (`specs/smena-front-object-matching`), не блокирует решение 4.
3. **Права доступа на запись `питДанныеПродажФронта`/вызов `ВыполнитьЗагрузкуДанных()` сверх базовой роли — не подтверждено чтением ролей.** См. `Critical Details → Контекст выполнения`: рабочая гипотеза (базовых прав «ответственного» достаточно, привилегированный режим не нужен) обоснована по аналогии с `Forms/ВозможностьЗагрузкиДПФ`, но не подтверждена прямым чтением состава ролей модуля «Общепит»/ERP, назначаемых на практике. Проверяется на тестовой ИБ при реализации; если прав не хватает — оркестрация решения 4 добавляет `УстановитьПривилегированныйРежим(Истина)` на время своей работы (стандартный приём для нового кода общего модуля, не требует пересмотра архитектуры).
4. **Точная классификация ошибок формирования по тексту сообщений `питЗагрузкаИзДанныхПродажФронта`.** Полный текст `ObjectModule.bsl` этой обработки превышает лимит чтения инструмента (668666 символов) и не был вычитан целиком ни в этой, ни в предыдущей фазе — известна только сигнатура `ВыполнитьЗагрузкуДанных()` и общая оркестрация. Классификация ошибок на категории «нехватка марок/акта» / «нехватка ингредиентов» / «прочее» (`specs/smena-target-document-generation`) в этом design.md описана на уровне подхода (парсинг `ПолучитьСообщенияПользователю`), но точные тексты/структуры сообщений, которые реально генерирует эта обработка, не подтверждены построчным чтением — уточняется на этапе реализации по наблюдаемому поведению на тестовых данных.
5. **Граница диапазона сканирования для `РекомендуемыйДень`.** `proposal.md` не фиксирует горизонт (сколько дней в прошлое сканировать), см. `Critical Details`. Не блокирует реализацию (дефолт есть), но точный дефолт — предмет уточнения на реальных объёмах данных заказчика.
6. **Из `proposal.md → Open Questions` (перенесено без изменений, не переоткрывается):** интерактивные исправления отрицательных остатков внутри мастера — остаётся `report_only` в этой версии; вопрос про будущие интерактивные сценарии исправления не в scope `design.md`.
