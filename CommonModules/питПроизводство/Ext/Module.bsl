...
//  ПараметрыУказанияСерий			 - Структура - параметры указания серий, возвращаемые соотвествующей процедурой модуля менеджера документа.
//
Процедура ЗаполнитьСерииПоFEFO(Объект,ПараметрыУказанияСерий,СтатусыУказанияСерийЗаполнены = Истина, тблСозданныеДокументы = Неопределено) Экспорт
	...
	Если ПараметрыУказанияСерий.ИмяТЧТовары = ПараметрыУказанияСерий.ИмяТЧСерии Тогда
		...
		MRS_ЗагрузкаЕГАИСФронт.ЗаполнитьМаркиПоОстаткамИСВТЧТовары(Объект, ПараметрыУказанияСерий, тблСозданныеДокументы); 
		//<<
	Иначе
