...
Процедура ФоновоеЗаданиеЗагрузкаЧеков(ФронтСистема, Пользователь) Экспорт
	...
	Иначе
		...
		ЗаписьЖурналаРегистрацииОбмена("Загрузка чеков Micros", "Дата начала выгрузки больше или равна текущему дню.");
...
Процедура ЗаписатьВБазу(ТаблицаЗначенийORACLE, ПараметрыВыборки, СтруктураТаблицАкцизныхМарок = Неопределено) Экспорт //MRS-225
	...
	// внутри смешанного чека отрицательные строки считаем отменами, а не возвратами.
	НормализоватьОтменыВнутриЧековMICROS(ТаблицаЗначенийORACLE); 
...
// Нормализует строки чеков MICROS перед созданием документов "Чек общепита".
//
// В рамках одного Oracle-чека анализирует положительные и отрицательные строки.
// Если в чеке есть и положительные, и отрицательные количества, отрицательные строки
// считаются отменами строк продажи и вычитаются из положительных строк.
// Если после вычитания количество положительной строки становится равным нулю,
// строка удаляется из таблицы загрузки.
// Если в чеке все строки отрицательные, таблица не изменяется: такой чек считается возвратом.
//
// Параметры:
//  ТаблицаЗначенийORACLE - ТаблицаЗначений - таблица строк продаж, полученная из MICROS
//      и дополненная данными сопоставлений 1С.
//
Процедура НормализоватьОтменыВнутриЧековMICROS(ТаблицаЗначенийORACLE)

	Если ТаблицаЗначенийORACLE = Неопределено 
		ИЛИ ТаблицаЗначенийORACLE.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;

	// Важно: группируем именно по одному Oracle-чеку, без ВидОперации.
	ТаблицаЗначенийORACLE.Сортировать("ДатаГруппировка, НОМЕРЧЕКА, НОМЕРТОЧКИПРОДАЖ, ИДЧека, КОЛИЧЕСТВОТОВАРА Убыв");

	МассивСтрокКУдалению = Новый Массив;

	ИндексНачала = 0;

	Пока ИндексНачала < ТаблицаЗначенийORACLE.Количество() Цикл

		СтрокаНачала = ТаблицаЗначенийORACLE[ИндексНачала];

		МассивПоложительных = Новый Массив;
		МассивОтрицательных = Новый Массив;

		Индекс = ИндексНачала;

		Пока Индекс < ТаблицаЗначенийORACLE.Количество() Цикл

			СтрокаТаблицы = ТаблицаЗначенийORACLE[Индекс];

			Если НЕ ЭтоСтрокаТогоЖеЧекаMICROS(СтрокаНачала, СтрокаТаблицы) Тогда
				Прервать;
			КонецЕсли;

			Если СтрокаТаблицы.КОЛИЧЕСТВОТОВАРА > 0 Тогда
				МассивПоложительных.Добавить(СтрокаТаблицы);
			ИначеЕсли СтрокаТаблицы.КОЛИЧЕСТВОТОВАРА < 0 Тогда
				МассивОтрицательных.Добавить(СтрокаТаблицы);
			КонецЕсли;
		...
		// Если есть и плюсовые, и минусовые строки — это отмены внутри чека.
		// Если все строки отрицательные — ничего не делаем, это реальный возврат.
		Если МассивПоложительных.Количество() > 0 
			И МассивОтрицательных.Количество() > 0 Тогда

			ОбработатьОтменыВнутриЧекаMICROS(МассивПоложительных, МассивОтрицательных, МассивСтрокКУдалению);
	...
	// Удаляем отрицательные строки-отмены и положительные строки, которые полностью схлопнулись в 0.
	Для Каждого СтрокаКУдалению Из МассивСтрокКУдалению Цикл
		Попытка
			ТаблицаЗначенийORACLE.Удалить(СтрокаКУдалению);
		Исключение
			// Строка могла быть уже удалена, пропускаем.
		КонецПопытки;
	КонецЦикла;

	ТаблицаЗначенийORACLE.Сортировать("ДатаГруппировка, НОМЕРЧЕКА, НОМЕРТОЧКИПРОДАЖ, НомерПозицииВЧеке");

КонецПроцедуры

// Обрабатывает отрицательные строки внутри одного смешанного чека MICROS.
//
// Для каждой отрицательной строки подбирает положительную строку этого же чека.
// Сначала подбор выполняется по номенклатуре/коду товара и цене, затем,
// если строка не найдена, только по номенклатуре/коду товара.
// Найденное отрицательное количество вычитается из положительной строки.
// Отрицательные строки, обработанные как отмены, добавляются в массив строк к удалению.
//
// Параметры:
//  МассивПоложительных - Массив - положительные строки одного чека.
//  МассивОтрицательных - Массив - отрицательные строки этого же чека.
//  МассивСтрокКУдалению - Массив - массив строк таблицы загрузки,
//      которые должны быть удалены после завершения обработки.
//
Процедура ОбработатьОтменыВнутриЧекаMICROS(МассивПоложительных, МассивОтрицательных, МассивСтрокКУдалению)

	Для Каждого СтрокаОтмены Из МассивОтрицательных Цикл

		КоличествоКОтмене = -СтрокаОтмены.КОЛИЧЕСТВОТОВАРА;
		ИсходноеКоличествоОтмены = КоличествоКОтмене;

		// Первый проход: ищем положительную строку по номенклатуре/коду и цене.
		Для Каждого СтрокаПродажи Из МассивПоложительных Цикл

			Если КоличествоКОтмене <= 0 Тогда
				Прервать;
			КонецЕсли;

			Если НЕ СтрокиСовпадаютДляОтменыMICROS(СтрокаПродажи, СтрокаОтмены, Истина) Тогда
				Продолжить;
			КонецЕсли;

			КоличествоКОтмене = СписатьКоличествоОтменыMICROS(
									СтрокаПродажи,
									СтрокаОтмены,
									КоличествоКОтмене,
									ИсходноеКоличествоОтмены,
									МассивСтрокКУдалению);

		КонецЦикла;

		// Второй проход: если по цене не нашли, пробуем только по номенклатуре/коду.
		// Это нужно на случай округлений или свободной цены.
		Если КоличествоКОтмене > 0 Тогда
			...
			Для Каждого СтрокаПродажи Из МассивПоложительных Цикл
				...
				Если НЕ СтрокиСовпадаютДляОтменыMICROS(СтрокаПродажи, СтрокаОтмены, Ложь) Тогда
					Продолжить;
				КонецЕсли;
		...
		Если КоличествоКОтмене > 0 Тогда
			...
			ТекстОшибки = СтрШаблон(
				"Не удалось сопоставить отмену внутри чека MICROS. Чек: %1, точка продаж: %2, код товара: %3, количество отмены: %4",
				СтрокаОтмены.НОМЕРЧЕКА,
				СтрокаОтмены.НОМЕРТОЧКИПРОДАЖ,
				СтрокаОтмены.КОДТОВАРА,
				КоличествоКОтмене);

			ЗаписьЖурналаРегистрацииОбмена("Загрузка чеков Micros", ТекстОшибки, Истина);

			// Лучше падать, чем молча сделать неверный чек.
			ВызватьИсключение ТекстОшибки;

		КонецЕсли;

		// Отрицательная строка в смешанном чеке — это отмена, в документ она не должна попадать.
		Если МассивСтрокКУдалению.Найти(СтрокаОтмены) = Неопределено Тогда
			МассивСтрокКУдалению.Добавить(СтрокаОтмены);
		КонецЕсли;
...
// Вычитает количество отмены из положительной строки продажи MICROS.
//
// Уменьшает количество, сумму строки, сумму скидки строки и пересчитывает цену.
// Если после вычитания количество положительной строки становится равным нулю,
// добавляет эту строку в массив строк к удалению.
//
// Параметры:
//  СтрокаПродажи - СтрокаТаблицыЗначений - положительная строка продажи.
//  СтрокаОтмены - СтрокаТаблицыЗначений - отрицательная строка отмены.
//  КоличествоКОтмене - Число - остаток количества отмены, который нужно вычесть.
//  ИсходноеКоличествоОтмены - Число - исходное количество отрицательной строки
//      без знака минус. Используется для расчета доли суммы и скидки.
//  МассивСтрокКУдалению - Массив - массив строк таблицы загрузки,
//      которые должны быть удалены после завершения обработки.
//
// Возвращаемое значение:
//  Число - остаток количества отмены, который не удалось списать с текущей строки продажи.
//
Функция СписатьКоличествоОтменыMICROS(СтрокаПродажи, СтрокаОтмены, КоличествоКОтмене, ИсходноеКоличествоОтмены, МассивСтрокКУдалению)

	Если СтрокаПродажи.КОЛИЧЕСТВОТОВАРА <= 0 Тогда
		Возврат КоличествоКОтмене;
	КонецЕсли;

	КоличествоСписать = Мин(СтрокаПродажи.КОЛИЧЕСТВОТОВАРА, КоличествоКОтмене);

	ДоляОтрицательнойСтроки = КоличествоСписать / ИсходноеКоличествоОтмены;
	ДоляПоложительнойСтроки = КоличествоСписать / СтрокаПродажи.КОЛИЧЕСТВОТОВАРА;

	// Количество уменьшаем.
	СтрокаПродажи.КОЛИЧЕСТВОТОВАРА = СтрокаПродажи.КОЛИЧЕСТВОТОВАРА - КоличествоСписать;

	// Сумму лучше уменьшать по отрицательной строке, потому что именно она пришла из MICROS.
	СтрокаПродажи.СУММАТОВАРА = ЧислоMICROS(СтрокаПродажи.СУММАТОВАРА) + ЧислоMICROS(СтрокаОтмены.СУММАТОВАРА) * ДоляОтрицательнойСтроки;

	// Скидку также схлопываем. Если в MICROS у отмены скидка идет с нужным знаком,
	// эта логика даст правильную нетто-строку.
	СтрокаПродажи.СУММАСКИДКИСТРОКИ = ЧислоMICROS(СтрокаПродажи.СУММАСКИДКИСТРОКИ) + ЧислоMICROS(СтрокаОтмены.СУММАСКИДКИСТРОКИ) * ДоляОтрицательнойСтроки;

	Если СтрокаПродажи.КОЛИЧЕСТВОТОВАРА <> 0 Тогда
		СтрокаПродажи.ЦЕНАТОВАРА = Окр(ЧислоMICROS(СтрокаПродажи.СУММАТОВАРА) / СтрокаПродажи.КОЛИЧЕСТВОТОВАРА, 3);
	Иначе
		СтрокаПродажи.ЦЕНАТОВАРА = 0;
	КонецЕсли;

	Если СтрокаПродажи.КОЛИЧЕСТВОТОВАРА = 0 Тогда
		Если МассивСтрокКУдалению.Найти(СтрокаПродажи) = Неопределено Тогда
			МассивСтрокКУдалению.Добавить(СтрокаПродажи);
		КонецЕсли;
	КонецЕсли;

	Возврат КоличествоКОтмене - КоличествоСписать;

КонецФункции

// Проверяет, относятся ли две строки загрузки MICROS к одному Oracle-чеку.
//
// Сравнение выполняется по дате группировки, идентификатору Oracle-чека,
// номеру чека, точке продаж и фронтовой системе.
//
// Параметры:
//  Строка1 - СтрокаТаблицыЗначений - первая строка загрузки MICROS.
//  Строка2 - СтрокаТаблицыЗначений - вторая строка загрузки MICROS.
//
// Возвращаемое значение:
//  Булево - Истина, если строки относятся к одному Oracle-чеку;
//      Ложь, если строки относятся к разным чекам.
//
Функция ЭтоСтрокаТогоЖеЧекаMICROS(Строка1, Строка2)

	Возврат Строка1.ДатаГруппировка = Строка2.ДатаГруппировка
		И Строка1.ИДЧека = Строка2.ИДЧека
		И Строка1.НОМЕРЧЕКА = Строка2.НОМЕРЧЕКА
		И Строка1.НОМЕРТОЧКИПРОДАЖ = Строка2.НОМЕРТОЧКИПРОДАЖ
		И Строка1.ФронтСистема = Строка2.ФронтСистема;

КонецФункции

// Проверяет, может ли отрицательная строка MICROS быть отменой положительной строки.
//
// Основное сравнение выполняется по сопоставленной номенклатуре 1С.
// Если номенклатура не заполнена, используется код товара MICROS.
// При необходимости дополнительно сравнивается цена.
//
// Параметры:
//  СтрокаПродажи - СтрокаТаблицыЗначений - положительная строка продажи.
//  СтрокаОтмены - СтрокаТаблицыЗначений - отрицательная строка отмены.
//  УчитыватьЦену - Булево - признак дополнительного сравнения строк по цене.
//
// Возвращаемое значение:
//  Булево - Истина, если отрицательная строка может быть отменой указанной
//      положительной строки; Ложь, если строки не соответствуют друг другу.
//
Функция СтрокиСовпадаютДляОтменыMICROS(СтрокаПродажи, СтрокаОтмены, УчитыватьЦену)

	// Основной ключ — уже сопоставленная номенклатура.
	Если ЗначениеЗаполнено(СтрокаПродажи.Номенклатура)
		И ЗначениеЗаполнено(СтрокаОтмены.Номенклатура) Тогда

		Если СтрокаПродажи.Номенклатура <> СтрокаОтмены.Номенклатура Тогда
			Возврат Ложь;
		КонецЕсли;

	Иначе

		// Резервный ключ — код товара из MICROS.
		Если СтрокаПродажи.КОДТОВАРА <> СтрокаОтмены.КОДТОВАРА Тогда
			Возврат Ложь;
		КонецЕсли;

	КонецЕсли;

	Если УчитыватьЦену Тогда
		Если Окр(ЧислоMICROS(СтрокаПродажи.ЦЕНАТОВАРА), 2) <> Окр(ЧислоMICROS(СтрокаОтмены.ЦЕНАТОВАРА), 2) Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
// Возвращает числовое значение MICROS с заменой пустого значения на ноль.
//
// Используется при расчетах сумм и скидок, чтобы избежать ошибок при значениях
// NULL или Неопределено.
//
// Параметры:
//  Значение - Произвольный - исходное значение из строки загрузки MICROS.
//
// Возвращаемое значение:
//  Число - исходное значение, если оно заполнено; 0, если значение равно NULL
//      или Неопределено.
//
Функция ЧислоMICROS(Значение)
...
Функция ПолучениеДанныхORACLE(ПараметрыВыборки)
	...
	|        INNER JOIN (
	|            SELECT checkid, MAX(transactiontime) AS maxtime
	|            FROM transdb.totals
	|            GROUP BY checkid
	...
	|        SUM(CASE WHEN cd.detaillink < agg.detaillink THEN cd.total_sum ELSE 0 END)
	|            OVER (PARTITION BY cd.checkid)                                                                 AS total_for_discount_base
	...
	|    CAST(
	|        CASE
	|            WHEN cwdc.discount_detailindex IS NOT NULL
	|                 AND cwdc.detaillink >= cwdc.discount_detailindex THEN 0
	|            WHEN NVL(cwdc.total_for_discount_base, 0) = 0             THEN 0
	|            ELSE NVL(cwdc.total_sum, 0) / cwdc.total_for_discount_base
	|                 * NVL(-cwdc.misum, 0)
	|        END AS NUMBER(15, 3)
