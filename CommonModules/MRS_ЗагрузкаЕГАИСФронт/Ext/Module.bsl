...
//
Процедура ЗаполнитьМаркиПоОстаткамИСВТЧТовары(Объект, ПараметрыУказанияСерий, тблСозданныеДокументы = Неопределено) Экспорт
	...
	// >> [ERP-5907] Швецов. 17.06.26 -------------------------------------------------------
    // Уменьшаем свободный остаток по маркам, уже занятым в ранее созданных
    // документах ПроизводствоБезЗаказа (они ещё не проведены, поэтому
    // регистр их не видит).
    Если тблСозданныеДокументы <> Неопределено И тблСозданныеДокументы.Количество() > 0 Тогда

        МассивДокПроизводства = Новый Массив;
        Для Каждого СтрДок Из тблСозданныеДокументы Цикл
            Если ТипЗнч(СтрДок.ТиповойДокумент) = Тип("ДокументСсылка.ПроизводствоБезЗаказа") Тогда
                МассивДокПроизводства.Добавить(СтрДок.ТиповойДокумент);
            КонецЕсли;
        КонецЦикла;

        Если МассивДокПроизводства.Количество() > 0 Тогда

            ЗапросЗанятых = Новый Запрос;
            ЗапросЗанятых.Текст =
                "ВЫБРАТЬ
                |	МатериалыИРаботы.Номенклатура КАК Номенклатура,
                |	МатериалыИРаботы.Серия КАК Серия,
                |	СУММА(МатериалыИРаботы.Количество) КАК Количество
                |ИЗ
                |	Документ.ПроизводствоБезЗаказа.МатериалыИРаботы КАК МатериалыИРаботы
                |ГДЕ
                |	МатериалыИРаботы.Ссылка В(&МассивДокументов)
                |	И МатериалыИРаботы.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
                |
                |СГРУППИРОВАТЬ ПО
                |	МатериалыИРаботы.Номенклатура,
                |	МатериалыИРаботы.Серия";

            ЗапросЗанятых.УстановитьПараметр("МассивДокументов", МассивДокПроизводства);

            ВыборкаЗанятых = ЗапросЗанятых.Выполнить().Выбрать();

            Пока ВыборкаЗанятых.Следующий() Цикл

                МассивОстатков = СоответствиеОстатки[ВыборкаЗанятых.Номенклатура];
                Если МассивОстатков = Неопределено Тогда
                    Продолжить;
                КонецЕсли;

                Для Каждого СтруктураОстаток Из МассивОстатков Цикл
                    Если СтруктураОстаток.Серия <> ВыборкаЗанятых.Серия Тогда
                        Продолжить;
                    КонецЕсли;
                    // Количество в документе уже в литрах — единицы совпадают
                    СтруктураОстаток.СвободныйОстаток = СтруктураОстаток.СвободныйОстаток - ВыборкаЗанятых.Количество;
                    Если СтруктураОстаток.СвободныйОстаток < 0 Тогда
                        СтруктураОстаток.СвободныйОстаток = 0;
                    КонецЕсли;
                    Прервать;
                КонецЦикла;
...
// Получает дополнительный реквизит, запрещающий формировать Акт Списания ЕГАИС
//
// Параметры:
//   Пользователь - СправочникСсылка.Склады
//
// Возвращаемое значение:
//   Булево
//
Функция ПолучитьРеквизитРазрешение(Знач Склад)
	
	ЗначениеСвойства = УправлениеСвойствами.ЗначениеСвойства(Склад, "MRS_НеФормироватьСписаниеЕГАИС");
	ЗначениеСвойства = ?(ЗначениеСвойства = Неопределено, Ложь, ЗначениеСвойства);
...
Процедура ОбработатьПолученныеДанныеMICROS(ДанныеФискальнойБазы, ТаблицаМарокВДиапазаоне)
	...
	// Отфильтровываем некорректные марки
	Индекс = ДанныеФискальнойБазы.Количество() - 1;

	Пока Индекс >= 0 Цикл
		
		СтрокаБД = ДанныеФискальнойБазы[Индекс];
		КодМарки = Строка(СтрокаБД.code);
		...
		Иначе
			...
			Для Сч = 1 По ДлинаКода Цикл
				...
				СимволМарки = Сред(КодМарки, Сч, 1);
				КодТекущегоСимвола = КодСимвола(СимволМарки);
				
				// Допустимы только цифры и латинские буквы
				Если НЕ (
					(КодТекущегоСимвола >= 48 И КодТекущегоСимвола <= 57)
